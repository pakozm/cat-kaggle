...
if __name__ == '__main__':
    ...
    def generate_models():
        depths = [ 8,10,12, 9,11]
        rnds   = [42, 2,30,46,40]
        return [ ensemble.GradientBoostingRegressor(random_state = rnds[i],
                                                    n_estimators = 3500,
                                                    min_samples_split = 35,
                                                    max_depth = depths[i],
                                                    learning_rate = .01,
                                                    max_features  = 'sqrt',
                                                    loss ='ls' ) for i in range(len(depths)) ]
    ...
    for i in range(len(models)):
        ...
        pd.DataFrame({"cost": np.expm1(log_preds_val)}).to_csv('val_gbm%d.csv'%(i), index=True)
    ...
    for i in range(len(preds_test)):
        pd.DataFrame({"id": idx, "cost": preds_test[i]}).to_csv('test_gbm%d.csv'%(i), index=False)
