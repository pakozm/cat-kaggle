...
local name = "stage0_"
local train_mask = "val_%s*"%{name}
local test_mask  = "test_%s*"%{name}

local load_predictions = function(mask)
  return matrix.join( 2, iterator(glob(mask)):
                        map(lambda'|x| data_frame.from_csv(x)[{"cost"}]'):
                        map(lambda'|x| #x, 1, x'):
                        map(matrix):
                        call("log1p"):
                        table() )
...
local train          = load_predictions(train_mask)
local test           = load_predictions(test_mask)

--local train,center,scale = stats.standardize(train)
--local test = stats.standardize(test,{center=center,scale=scale})

local train_csv      = data_frame.from_csv("../input/train_set.csv")
local test_csv       = data_frame.from_csv("../input/test_set.csv")
local label          = train_csv:as_matrix("cost"):log1p()
local train_tube_ids = train_csv[{"tube_assembly_id"}]
local test_ids       = test_csv[{"id"}]

local label,label_center,label_scale =
  stats.standardize( label, {
                       center = matrix(1,1):fill(0), --matrix(1,1,{stats.percentile(label, 0.5)}), -- matrix(1,1):fill(0),
                       scale  = matrix(1,1):fill(1),
  } )

---------------------------------------------------------------------------

local B = 5 -- number of CV blocks
local bunch_size  = 128
local replacement = 2^13
local h1 = tonumber(arg[1] or 128)
local h2 = tonumber(arg[2] or 512)
local h3 = tonumber(arg[3] or 256)
local input_dropout = tonumber(0.0)
local h1_dropout = tonumber(0.0)
local h2_dropout = tonumber(0.0)
local h3_dropout = tonumber(0.1)
local lr = 1.0
local mt = tonumber(0.2)
local wd = tonumber(0.01)
local mp = 100.0
local mg = 100.0
local min_epochs =  40
local max_epochs = 400
local optimizer  = "adadelta"
local init_w     = tonumber(0.1)
local input_var  = tonumber(0.02)
...
local function renormalize(x)
  broadcast(x.cmul, x, label_scale, x)
  broadcast(bind(x.axpy, nil, 1.0), x, label_center, x)
  return x
end

local model = ann.mlp.all_all.generate(
  table.concat{
    "%d inputs "%{ train:dim(2) },
    "dropout{ prob=#2, random=#1 } ",
    h1>0 and ("%d relu dropout{ prob=#3, random=#1 } "%{ h1 }) or "",
    h2>0 and ("%d relu dropout{ prob=#4, random=#1 } "%{ h2 }) or "",
    h3>0 and ("%d relu dropout{ prob=#5, random=#1 } "%{ h3 }) or "",
    "1 linear" },
  { noise_rng, input_dropout, h1_dropout, h2_dropout, h3_dropout })
...
for i=1,B do
  ...
  local tr_input_ds = dataset.perturbation{
    dataset = tr_input_ds,
    random = noise_rng,
    variance = input_var,
  }
  ...
  while pocket:execute(function()
    ...
    print(pocket:get_state_string(), trainer:norm2("w.*"), trainer:norm2("b.*"), trainer:norm2("a.*"))
  end
...
do
  ...
  va_df:to_csv("STACK_MLPS/val_stage1.%d_%d_%d.%d_%d_%d_%d.csv"%{h1,h2,h3,data_shuffle_seed,shuffle_seed,weights_seed,noise_seed})
end
...
do
  ...
  test_df:to_csv("STACK_MLPS/test_stage1.%d_%d_%d.%d_%d_%d_%d.csv"%{h1,h2,h3,data_shuffle_seed,shuffle_seed,weights_seed,noise_seed})
end
